...
//        g2.setStroke(new BasicStroke(5));
     ...
     * @return
     */
    public String getPicture(String basePath, String filename, String verifyCode) {
        //获取日期文件夹
        String dataPath = MyDateFormat.format(new Date(), MyDateFormat.FORMAT_DATE_);
        //创建文件
        int w = 170, h = 70;

//        int w = 200, h = 85;
        File file = new File(basePath + File.separator + dataPath, filename + ".jpg");
        try {
            outputImage(w, h, file, verifyCode);
        } catch (IOException e) {
            e.printStackTrace();
        }
        return dataPath + File.separator + filename + ".jpg";
    }


	public static void main(String[] args) throws IOException{
		new VerifyCodeUtils().getPicture("/Volumes/wh/imageTest/verify","123","SXOU");
